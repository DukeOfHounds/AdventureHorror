    ...
    private IEnumerator IsActive()
    {
        ...
        if (FacingObjects.Count > 0f)
        {
            foreach (Facer item in FacingObjects)
            {
                ...
                Vector3 targetPostition = new Vector3(lookTarget.transform.position.x, item.item.transform.position.y, lookTarget.transform.position.z);
                item.item.transform.LookAt(targetPostition);
            }
        }
